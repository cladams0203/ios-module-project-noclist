...
func totalComprimised(nocList:[(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) -> Int{
    var comprimised = 0
    for spy in nocList{
        if spy.compromised == true{
            comprimised += 1
        }
    }
    return comprimised
...
print("\(totalComprimised(nocList: nocList)) agents have been comprimised!")
...
func findCleanAgents(nocList:[(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) -> [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]{
    var cleanArray: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)] = []
    for spy in nocList{
        if spy.compromised == false {
            print(spy.coverName)
            cleanArray.append(spy)
        }
    }
    return cleanArray
}
...
print("\(findCleanAgents(nocList: nocList).count) clean agents out of \(nocList.count) total agents")
...
func findHighRisk(nocList:[(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]){
    for spy in nocList{
        if spy.accessLevel > 7{
            if spy.compromised == true{
                print("\(spy.realName) ***WARNING*** ***COMPROMISED***")
            }else{
                print(spy.realName)
            }
        }
...
func levelCount(nocList: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]) {
    var low = 0
    var mid = 0
    var high = 0
    for spy in nocList{
        switch spy.accessLevel {
        case 0...4:
            low += 1
        case 5...7:
            mid += 1
    ...
    print("\(low) low level agents, \(mid) mid level agents, \(high) high level agents")
}

//: ## Step 10
//: Call the above function and check its output in the console.

levelCount(nocList: nocList)
...
func sortedSpy(nocList: [(coverName: String, realName: String, accessLevel: Int, compromised: Bool)]){
    let sortedArray = nocList.sorted(by: {$0.accessLevel < $1.accessLevel})
    for spy in sortedArray{
        print("cover Name: \(spy.coverName), access Level \(spy.accessLevel)")
    }
}

sortedSpy(nocList: nocList)
